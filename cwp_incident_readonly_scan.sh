...
BAD_WEB_RE='eval\(base64_decode|eval\(gzinflate|eval\(gzuncompress|base64_decode\(\$_(POST|REQUEST|COOKIE)|assert\(\$_(POST|REQUEST|COOKIE)|shell_exec\(|passthru\(|system\(|proc_open\(|popen\(|create_function\(|preg_replace\(.*\/e|FilesMan|c99|r57|wso|b374k|anonymousfox|foxwsov'
SUSPICIOUS_WEB_RE='__destruct|__call|call_user_func_array\(|call_user_func\(|file_put_contents\(|move_uploaded_file\(|chmod\(|curl_exec\('
FALSE_POSITIVE_RE='WSODs|data:image/png;base64|xlink:href="data:image'
grep -RIE "$SUSPICIOUS_WEB_RE" /home \
> "$OUT/web/suspicious_patterns_only.txt" 2>/dev/null || true
section() {
...
section "Hidden PHP Files"

find /home -type f \
\( -name '.*.php' -o -name '.*.phtml' \) \
> "$OUT/web/hidden_php_files.txt"

count_file "hidden_php_files" \
"$OUT/web/hidden_php_files.txt"

section "Recently Modified PHP"

find /home -type f \
\( -name '*.php' -o -name '*.phtml' \) \
-mtime -30 \
> "$OUT/web/recent_php_files_30d.txt"

count_file "recent_php_files_30d" \
"$OUT/web/recent_php_files_30d.txt"
