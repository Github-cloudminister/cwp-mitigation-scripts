...
QUAR="/root/malware-quarantine-$TS"
...
quarantine_file() {
    ...
    sha256sum "$f" >> "$BK/logs/quarantine_hashes.txt"
...
section "Remove Bash Profile Persistence"

find /root /home -type f \
\( -name '.bashrc' -o -name '.bash_profile' -o -name '.profile' \) \
-print 2>/dev/null > "$BK/logs/profile_files.txt"
...
while IFS= read -r f; do
    ...
    if grep -Eq \
    'defunct-kernel|/usr/bin/defunct|base64 -d\|bash|\[kswapd0\]' \
    "$f"
    then

        log "FOUND_PROFILE_PERSISTENCE: $f"
...
done < "$BK/logs/profile_files.txt"
...
section "Quarantine Suspicious Upload PHP"
...
-name '*.php' \
! -name 'index.php' \
-print > "$BK/logs/upload_php_review.txt"
...
while IFS= read -r f; do
    ...
    if echo "$base" | grep -Eq \
    '^[a-z]{8}\.php$|^[a-z]{7}\.php$|^[a-z]{6}\.php$'
    then

        log "SUSPICIOUS_RANDOM_UPLOAD_PHP: $f"

        quarantine_file "$f"

    fi

done < "$BK/logs/upload_php_review.txt"
...
find /root /home /etc/sudoers.d /usr/local/cwpsrv \
  -xdev \
  -exec lsattr {} + 2>/dev/null \
  | grep 'i' \
  > "$BK/logs/immutable_files.txt" || true

log "immutable_files=$(wc -l < "$BK/logs/immutable_files.txt")"
